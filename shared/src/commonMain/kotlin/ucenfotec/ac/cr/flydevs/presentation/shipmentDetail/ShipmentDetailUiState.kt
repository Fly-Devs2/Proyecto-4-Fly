...
data class ShipmentDetailUiState(
    ...
    val totalOrders: Int get() = batch?.orderIds?.size ?: 0
    ...
    val deliveryEvidenceUrl: String?
        get() = batch?.deliveryEvidence?.downloadUrl?.takeIf { it.isNotBlank() }

    val pickupEvidenceUrl: String?
        get() = batch?.pickupEvidence?.downloadUrl?.takeIf { it.isNotBlank() }
